...
| MCP | Required | Role |
|---|---|---|
| [@shuji-bonji/pdf-verify-mcp](https://www.npmjs.com/package/@shuji-bonji/pdf-verify-mcp) | **Yes** | Signature verification, tamper detection (**object-level revision diff from v0.10.0**), PAdES level, PDF/A validation (**PDF/A-4 from v0.11.0**). **v0.21.0 or later is recommended.** Three things changed. v0.15.0 stopped swallowing a `/Prev` link that could not be followed — before it, a document with 8 revisions and 5 signatures came back as having one, so a report promising the *full* revision history could not be written at all. v0.16.0 made the answer a field, `revisionChain: { status, missing }`; until then the only signal was English prose in `notes`, and this skill decided whether it could promise a full history by matching sentences. v0.17.0 added `revisionCountAgreement: { status, causes }` — the `startxref` count and the listed revisions differ lawfully (linearisation, an incomplete chain), and this field says whether the difference is explained; `unaccounted` means the file holds a `startxref` the walked chain does not reach, which is the case worth opening the file over. **v0.20.0 put `scope` — the scope of the reading, not a verdict — at the head of every report**: `reconstructed: true` means the cross-reference table was rebuilt by the tool and is not the one the file carries, so the signature list may be incomplete. **v0.20.0 also changed the `code` of a refusal that names a clause from `INTERNAL_ERROR` to `PARSE_FAILED`** — before that, "this document violates §7.5.4" and "the server crashed" came back under the same code, and this skill filed the former under "checks not performed". **v0.21.0 changed the top level of `verify_signatures` and `detect_pades_level` from an array to an object** (`{ scope, signatures }` / `{ scope, levels }`). The prose fallbacks for older versions are written out in `SKILL.md`. |
