...
2. `verify_integrity` — 増分更新の全履歴を報告に含める（どの時点で何が足されたかは
   ...
   🔴 **「全履歴」と書けるのは `revisionChain.status` が `complete` のときだけ。**
   `revisions[]` が返ったことは「全件を歩けた」ことを意味しない（判定の仕方と旧版での
   退避は SKILL.md「Phase 2.5」の 2）。`partial` なら書けるのは
   **「辿れた範囲の履歴」**であって全履歴ではなく、`missing` が欠けた端を名指す。
   証拠として提出する文書でこの 2 つを取り違えると、
   **「この時点以降は何も足されていない」という、していない主張をしたことになる**。
