...
2. `verify_integrity` — 増分更新の全履歴を報告に含める（どの時点で何が足されたかは
   ...
   **`revisions[]` が返ったことは「全件を歩けた」ことを意味しない。** チェーンが途中で
   切れた場合も、最新セクションが読めず古い入口から入った場合も、リストは短いまま返る。
   どちらも `notes` にしか出ないので（真偽のフィールドは無い）、**「全履歴」と書く前に
   必ず `notes` を読む**。打ち切りが立っていたら、書けるのは「辿れた範囲の履歴」であって
   全履歴ではない。証拠として提出する文書で、この 2 つを取り違えると
   **「この時点以降は何も足されていない」という、していない主張をしたことになる**。

   **verify v0.15.0 未満では、この確認をしても足りない** — 追えない `/Prev 0` を
   「前のセクションは無い」として飲み込み、打ち切りを立てないまま完全なチェーンとして
   返していたため（8 リビジョン 5 署名の実検体が「1 リビジョン」として報告された）。
   全履歴を要求する本プロファイルでは **v0.15.0 以降で測り直す**
