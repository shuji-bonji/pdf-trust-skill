...
3. `verify_integrity` — 署名後変更は原則すべて注記（検査値の書き換えは一文字でも重大）。
   **verify v0.10.0+ なら `objectChangesAfterLastSignature` でオブジェクト番号・型・役割まで、
   ...
   🔴 **「原則すべて」を書く前に `revisionChain.status` を確かめる**（SKILL.md「Phase 2.5」の 2）。
   `complete` でないときの `revisions[]` は短いまま返るので、**一覧に検査値の書き換えが
   出てこないことを「書き換えは無かった」の根拠にできない**。診療文書では見落としが
   患者に及ぶので、歩けなかった範囲は「未確認」として明示する。
