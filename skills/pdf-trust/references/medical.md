...
3. `verify_integrity` — 署名後変更は原則すべて注記（検査値の書き換えは一文字でも重大）。
   ...
   **「原則すべて」を書く前に `notes` でチェーンが歩き切れているか確かめる。** 打ち切りが
   立っているときの `revisions[]` は短いまま返るので、**一覧に検査値の書き換えが出てこない
   ことを「書き換えは無かった」の根拠にできない**。診療文書では見落としが患者に及ぶので、
   歩けなかった範囲は「未確認」として明示する。**verify v0.15.0 未満は打ち切り自体を
   見落とすことがある**ため、この版以降で測る
